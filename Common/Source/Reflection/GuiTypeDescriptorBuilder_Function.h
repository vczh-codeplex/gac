...
***********************************************************************/
			...
			namespace internal_helper
			{
				...
				{
					UnboxParameter<typename TypeInfoRetriver<T0>::TempValueType>(arguments->Get(0), p0, index, L"nth-argument");
					UnboxSpecifiedParameter(arguments, index + 1, args);
				}

				template<typename R, typename ...TArgs>
				Value InvokeBoxedFunction(const Func<R(TArgs...)>& function, Ptr<IValueList> arguments, TArgs&& ...args)
				{
					UnboxSpecifiedParameter<TArgs>(arguments, 0, args);
			...
			public:
				...
				Value Invoke(Ptr<IValueList> arguments)override
				{
					...
					return internal_helper::InvokeBoxedFunction<R, TArgs...>(function, arguments, TArgs()...);
				}
			};
