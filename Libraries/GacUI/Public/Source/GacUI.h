...
Comments:
  WinDC::PolyDraw
	...
	Actions				:???????????????,???????:
		PT_MOVETO		:??1????MoveTo
		PT_LINETO		:??1????LineTo
		PT_BEZIERTO		:??3????Bezier??
		?????????PT_CLOSEFIGURE??or??,???????????PT_MOVETO?MoveTo??????????
	PointCount			:????????
  WinDC::DrawBuffer
    Format: ??DrawText??
  Pen:
	Style:
		PS_SOLID?PS_DASH?PS_DOT?PS_DASHDOT?PS_DASHDOTDOT?PS_USERSTYLE(Geometric????)
	EndCap:
  ...
  WinDIB:
	TransformAlphaChannel():	??????alpha???alpha??????alpha??
	Generate×××():			???????alpha???alpha???
	?WinDC?Draw?????,alpha????alpha?????????
*******************************************************************************/
